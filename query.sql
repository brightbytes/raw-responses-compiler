SELECT   *
FROM     (SELECT   DISTINCT (q.congo_survey_id) AS id,
          ...
          AND      o.entity_type = 'School') AS all_campaigns
WHERE    all_campaigns.group_name IS NOT NULL
ORDER BY all_campaigns.organization_name,
         (CASE WHEN all_campaigns.group_name LIKE '%Teacher%' THEN 1
               WHEN all_campaigns.group_name LIKE '%Lower%'   THEN 2
               WHEN all_campaigns.group_name LIKE '%Middle%'  THEN 3
               WHEN all_campaigns.group_name LIKE '%High%'    THEN 4
               WHEN all_campaigns.group_name LIKE '%Parent%'  THEN 5
               ELSE 6
               END);
